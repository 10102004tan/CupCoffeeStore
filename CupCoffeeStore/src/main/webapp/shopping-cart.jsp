                                ...
                                <tr>
                                    <td class="product__cart__item">
                                        <div class="product__cart__item__pic" >
                                            <img class="image-fluid" style="width: 70px; height: 70px;object-fit: cover" src="images/<%=order.getSanPham().getHinh()%>" alt="">
                                        </div>
                                        <div class="product__cart__item__text">
                                            <h6><%=order.getSanPham().getTen()%></h6>
                                            <h5 class="format-price"><%=order.getSanPham().getGia()%></h5>
                                        </div>
                                    </td>
                                    <td class="quantity__item">
                                        <div class="quantity d-flex">
                                           <input class="input-quantity w-25" type="number" value="<%=order.getSoLuong()%>" data-value="<%=order.getMa()%>"  name="soLuong" >
                                        </div>
                                    </td>
                                    <td class="cart__price format-price"><%=order.getGia()*order.getSoLuong() %></td>
                                    <td class="cart__close"><i class="fa fa-close" style="cursor: pointer;" data-value="<%=order.getMa()%>"></i></td>
                                </tr>
                                <%} %>
                                <%} %>
                            </tbody>
	...
	cartCloses.forEach(element => {
		element.addEventListener('click', function(){
			$.ajax({
				...
				type : "POST",
				data : {
					ma : element.getAttribute('data-value'),
				},
